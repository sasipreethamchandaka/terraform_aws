data "aws_ami" "fornt-server" {
  most_recent = true
  owners      = ["self"] # Replace with the AWS account ID if needed

  filter {
    name   = "name"
    values = ["fornt-server"] # Use your AMI name pattern
  }
}
...
resource "aws_launch_template" "frontend" {
  ...
  image_id = data.aws_ami.fornt-server.id
...
###################################################################################
data "aws_ami" "back-server" {
  most_recent = true
  owners      = ["self"] # Replace with the AWS account ID if needed

  filter {
    name   = "name"
    values = ["back-server"] # Use your AMI name pattern
  }
}
...
resource "aws_launch_template" "backend" {
  ...
  image_id = data.aws_ami.back-server.id
  instance_type = "t2.micro"
